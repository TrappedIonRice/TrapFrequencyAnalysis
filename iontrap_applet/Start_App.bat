...
REM ---- Find repo root by looking for iontrap_applet\app.py ----
set "SCRIPT_DIR=%~dp0"
set "REPO_ROOT="

for %%D in ("%SCRIPT_DIR%." "%SCRIPT_DIR%.." "%SCRIPT_DIR%..\..") do (
  if exist "%%~fD\iontrap_applet\app.py" (
    set "REPO_ROOT=%%~fD"
    goto :FOUND_ROOT
  )
)

echo [error] Could not locate repo root (iontrap_applet\app.py not found nearby).
...
"%VENV_PY%" -m pip install -r "iontrap_applet\requirements-app.txt"
if errorlevel 1 (
...
REM ---- Run app ----
"%VENV_PY%" -m streamlit run "iontrap_applet\app.py"
